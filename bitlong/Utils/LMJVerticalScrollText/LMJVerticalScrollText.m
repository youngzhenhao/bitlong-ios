...
-(void)setNumOfLines:(NSInteger)numOfLines{
    ...
    _currentScrollLabel.numberOfLines = numOfLines;
    _standbyScrollLabel.numberOfLines = numOfLines;
}
...
- (void)createScrollLabelNeedStandbyLabel:(BOOL)isNeed{
    ...
    _currentScrollLabel.numberOfLines = _numOfLines;
    ...
    if (isNeed) {
        ...
        _standbyScrollLabel.numberOfLines = _numOfLines;
...
- (void)scrollWithNoSpaceByDirection:(NSNumber *)direction{
    ...
    if (![self isCurrentViewControllerVisible:[self viewController]]) {
        [self performSelector:@selector(scrollWithNoSpaceByDirection:) withObject:direction afterDelay:0.5f];
        ...
        [UIView animateWithDuration:_scrollAnimationTime delay:_textStayTime options:UIViewAnimationOptionLayoutSubviews animations:^{
            ...
            self->_currentScrollLabel.frame = CGRectMake(0, -self.lmj_height*direction.integerValue, self->_currentScrollLabel.lmj_width, self->_currentScrollLabel.lmj_height);
            self->_standbyScrollLabel.frame = CGRectMake(0, 0, self->_standbyScrollLabel.lmj_width, self->_standbyScrollLabel.lmj_height);
...
- (void)scrollWithSpaceByDirection:(NSNumber *)direction{
    ...
    if (![self isCurrentViewControllerVisible:[self viewController]]) {
        ...
        [UIView animateWithDuration:_scrollAnimationTime/2.f delay:_textStayTime options:UIViewAnimationOptionLayoutSubviews animations:^{
            ...
            self->_currentScrollLabel.frame = CGRectMake(0, self.lmj_height*direction.integerValue, self->_currentScrollLabel.lmj_width, self->_currentScrollLabel.lmj_height);
            self->_index = [self nextIndex:self->_index];
            self->_currentScrollLabel.lmj_text  = self->_textDataArr[self->_index];
            
            
            [UIView animateWithDuration:self->_scrollAnimationTime/2.f animations:^{
                self->_currentScrollLabel.frame = CGRectMake(0, 0, self->_currentScrollLabel.lmj_width, self->_currentScrollLabel.lmj_height);
