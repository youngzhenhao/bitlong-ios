...
#define kCycleScrollViewInitialPageControlDotSize CGSizeMake(4.5, 4.5)
...
//    int targetIndex = currentIndex + 1;
    int targetIndex = [self pageControlIndexWithCurrentCellIndex:currentIndex]+1;
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if (_pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
            [self.pageControl mas_makeConstraints:^(MASConstraintMaker *make) {
                make.right.equalTo(self).offset(-_pageControlRightOffset);
                make.bottom.offset(-self.pageControlBottomOffset);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @autoreleasepool {
        ...
        if (!cell.hasConfigured) {
            ...
            cell.maskView.hidden = !_displayMask;
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    CGFloat offset = 0.0;
    if (contentOffset.x <= _flowLayout.itemSize.width) {
        offset = contentOffset.x/_flowLayout.itemSize.width;
    }else{
        offset = (contentOffset.x - _flowLayout.itemSize.width * _currentRow)/_flowLayout.itemSize.width;
    }
    
    NSInteger currentIndx = (indexOnPageControl == 0 ? self.titlesGroup.count-1 : indexOnPageControl-1);
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:offset:currentIndx:nextIndex:isTurnFirst:)]) {
        if (indexOnPageControl == 0 && currentIndx == -1) {
//            NSLog(@"%.2f,%ld,%ld,%d",offset,currentIndx,(long)indexOnPageControl,YES);
            [self.delegate cycleScrollView:self offset:offset currentIndx:currentIndx nextIndex:indexOnPageControl isTurnFirst:YES];
        }else{
//            NSLog(@"%.2f,%d,%ld,%d",offset,indexOnPageControl-1,(long)indexOnPageControl,NO);
            if ((indexOnPageControl-1 < 0 && offset < 0) || indexOnPageControl == self.titlesGroup.count-1) {
            }else{
                [self.delegate cycleScrollView:self offset:offset currentIndx:indexOnPageControl-1 nextIndex:indexOnPageControl isTurnFirst:NO];
            }
        }
    }
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    if (self.autoScroll) {
        ...
        int itemIndex = [self currentIndex];
        int indexOnPageControl = [self pageControlIndexWithCurrentCellIndex:itemIndex];
        self.currentRow=indexOnPageControl;
        ...
        if (self.itemDidScrollOperationBlock) {
            self.itemDidScrollOperationBlock(indexOnPageControl,scrollView,_mainView);
        }
    }
}
...
- (void)makeScrollViewScrollToIndex:(NSInteger)index{
    ...
    if (index==-1) {
        ...
        [self scrollToIndex:(int)(self.currentRow)];
    }else{
