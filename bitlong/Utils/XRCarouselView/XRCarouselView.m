...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            else [_images addObject:[UIImage imageNamed:@"placeholder" inBundle:[NSBundle bundleForClass:[self class]] compatibleWithTraitCollection:nil]];
...
- (void)nextPage {
    if (_changeMode == ChangeModeFade) {
        ...
        [UIView animateWithDuration:1.2 animations:^{
            weakSelf.currImageView.alpha = 0;
            weakSelf.otherImageView.alpha = 1;
            weakSelf.pageControl.currentPage = weakSelf.nextIndex;
        } completion:^(BOOL finished) {
            [weakSelf changeToNext];
        }];
