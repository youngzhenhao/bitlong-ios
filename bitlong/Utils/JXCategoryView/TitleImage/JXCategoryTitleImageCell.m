...
- (void)layoutSubviews {
    ...
    switch (myCellModel.imageType) {
        ...
        case JXCategoryTitleImageType_RightImage:
        {
            ...
            self.imageView.center = CGPointMake(CGRectGetMaxX(self.titleLabel.frame) + myCellModel.titleImageSpacing + imageSize.width, self.contentView.center.y);
        }
            break;
            
        case JXCategoryTitleImageType_BelowImage:
        {
            [self refreshTitleLabelCenter:CGPointMake(self.titleLabel.bounds.size.width/2 + myCellModel.titleImageDistance, CGRectGetMaxY(self.imageView.frame) - self.titleLabel.bounds.size.height/2 - myCellModel.titleImageDistance/2)];
...
- (void)reloadData:(JXCategoryBaseCellModel *)cellModel {
    ...
    if (currentImageName != nil && ![currentImageName isEqualToString:self.currentImageName]) {
        ...
        if ([currentImageName hasPrefix:@"http://"] || [currentImageName hasPrefix:@"https://"]) {
        }else{
    ...
    self.imageView.layer.borderWidth = myCellModel.imageBordeWidth;
    self.imageView.layer.borderColor = myCellModel.imageBordeColor.CGColor;
